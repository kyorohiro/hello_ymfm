...
import { YM2612DirectTransport, YM2612Synth } from "./ym2612synth.js";
...
const CONTROL_IDS = ["dt", "multi", "tl", "ar", "d1r", "d2r", "sl", "rr", "algorithm", "feedback"];
const controls = Object.fromEntries(CONTROL_IDS.map((id) => [id, document.getElementById(id)]));
const values = Object.fromEntries(CONTROL_IDS.map((id) => [id, document.getElementById(`${id}Value`)]));
...
const voices = Array.from({ length: VOICE_COUNT }, (_, channel) => ({
  channel,
  held: false,
  key: null,
  startedAt: 0,
...
function createPitchFromMidi(midi) {
  ...
  let fnum = REFERENCE_FNUM * Math.pow(2, (midi - REFERENCE_MIDI) / 12);
  ...
  return {
    block,
    fnum: Math.max(0, Math.min(0x7ff, Math.round(fnum))),
  };
}
...
function applyPatchToVoices() {
  ...
  for (let channel = 0; channel < VOICE_COUNT; channel += 1) {
    synth.setOperator(channel, 1, { tl: 0x7f, multi: 1, ar: 31, d1r: 0, d2r: 0, sl: 0, rr: 15 });
    synth.setOperator(channel, 2, { tl: 0x7f, multi: 1, ar: 31, d1r: 0, d2r: 0, sl: 0, rr: 15 });
    synth.setOperator(channel, 3, { tl: 0x7f, multi: 1, ar: 31, d1r: 0, d2r: 0, sl: 0, rr: 15 });
    ...
    synth.setAlgo(channel, patchState.algorithm, patchState.feedback);
...
function updateKeyboardVisuals() {
  for (const button of keyboard.querySelectorAll(".key")) {
    button.classList.toggle("is-active", activeKeys.has(button.dataset.key));
  }
}
...
function chooseVoice() {
  const freeVoice = voices.find((voice) => !voice.held);
  if (freeVoice) {
    return freeVoice;
  }
...
async function ensureAudioReady() {
  ...
  // これは毎回 current state を見る
  if (audioContext.state !== "running") {
    await audioContext.resume();
  }

  // 重い初期化だけ Promise で一度に制限
  if (!audioReadyPromise) {
    audioReadyPromise = (async () => {
      ...
      if (!processor) {
        processor = audioContext.createScriptProcessor(1024, 0, 2);
        ...
        processor.onaudioprocess = (event) => {
          ...
          const { left, right } = ym2612.generateStereo(leftOut.length);
          leftOut.set(left);
          rightOut.set(right);
        };
  ...
  setStatus(`Audio ready. YM2612 at ${sampleRate} Hz.`);
...
async function pressKey(key) {
  ...
  const entry = KEY_LAYOUT.find((item) => item.key === key);
  if (!entry || activeKeys.has(key)) {
    return;
  }
  ...
  if (voice.held) {
    ...
    if (voice.key) {
      activeKeys.delete(voice.key);
    }
  }

  synth.noteOn(voice.channel, entry.pitch.block, entry.pitch.fnum);
  voice.held = true;
  voice.key = key;
  voice.startedAt = performance.now();
  activeKeys.set(key, voice.channel);
  updateKeyboardVisuals();
  setStatus(`Playing ${entry.noteName} on channel ${voice.channel + 1}.`);
}
...
function releaseKey(key) {
  ...
  const channel = activeKeys.get(key);
...
function buildKeyboard() {
  ...
  for (const row of ROW_DEFS) {
    ...
    rowElement.dataset.count = String(row.keys.length);

    for (const keyChar of row.keys) {
      const entry = KEY_LAYOUT.find((item) => item.key === keyChar);
...
for (const id of CONTROL_IDS) {
  controls[id].addEventListener("input", () => {
    patchState[id] = Number(controls[id].value);
    syncControlLabels();
    renderCodeSample();
    if (synth) {
      applyPatchToVoices();
...
window.addEventListener("keydown", (event) => {
  ...
  if (!KEY_LAYOUT.some((entry) => entry.key === key)) {
    return;
  }
  event.preventDefault();
  if (event.repeat) {
